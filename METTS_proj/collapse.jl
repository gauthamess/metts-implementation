...
#OLDCPS
function cpscollapse2(metts, base)
    L = size(metts, 1)
    ...
    for i in 1:L
        ...
        for j in 1:length(pm)
          if proll < sumprob[j] #then outcome is j-2
            m = j-2
            metts = sitecanonical(metts,i, tol) #sitecanonical at site i

            p_i = pm[j]

            bond_right = metts[i][1,j,:]/pm[j]
            if i < L
...
function cpscollapse(metts, base)
    ...
    for i in 1:L
        ...
        for j in 1:length(pm)
          if proll < sumprob[j]
            P = pm[j]^(-1/2) * proj(j-2, base)
            mpo = proj_mpo(P, L, i)

            metts = mpo_on_mps(mpo, metts)
